...
// Safety check: In production, refresh token must use a different secret than access token
const isProduction = process.env.NODE_ENV === "production";
if (isProduction && !process.env.JWT_REFRESH_SECRET) {
  console.error(
    "[SECURITY] JWT_REFRESH_SECRET is not set in production. Refresh tokens will be insecure.",
  );
}

const EMAIL_VERIFICATION_TTL_MINUTES = parseInt(
  process.env.EMAIL_VERIFICATION_TTL_MINUTES || "30",
  10,
);
const EMAIL_RESEND_COOLDOWN_SECONDS = parseInt(
  process.env.EMAIL_RESEND_COOLDOWN_SECONDS || "60",
  10,
);
const VERIFICATION_TOKEN_PATTERN = /^[a-f0-9]{64}$/i;
const EMAIL_CHANGE_TOKEN_SECRET =
  process.env.EMAIL_CHANGE_TOKEN_SECRET || accessTokenFallbackSecret;
...
const generateRefreshToken = (payload) => {
  ...
  // Safety: In production, require explicit JWT_REFRESH_SECRET
  const secret = refreshTokenFallbackSecret || (isProduction ? null : accessTokenFallbackSecret);
  if (!secret) {
    throw new Error("JWT_REFRESH_SECRET is required in production environment");
  }
  if (!refreshTokenFallbackSecret && !isProduction) {
    console.warn("[SECURITY] Using JWT_SECRET as fallback for refresh tokens. Set JWT_REFRESH_SECRET for better security.");
  }
...
const issueVerificationEmail = async (customer, options = {}) => {
  ...
  if (enforceCooldown) {
    ...
    if (latestToken) {
      const ageInSeconds = Math.floor(
        (Date.now() - new Date(latestToken.created_at).getTime()) / 1000,
      );
      const retryAfterSeconds = Math.max(
        EMAIL_RESEND_COOLDOWN_SECONDS - ageInSeconds,
        0,
      );
  ...
  const { token, tokenHash } = await createEmailVerificationToken(
    customer.CustomerID,
  );
...
const verifyCustomerEmailChangeToken = async (rawToken, res) => {
  ...
  try {
    ...
    if (error?.name === "TokenExpiredError") {
      return res.status(400).json({
        error:
          "Email change link expired. Request a new email change from your profile settings.",
  ...
  const currentEmail = String(decoded?.currentEmail || "")
    .trim()
    .toLowerCase();
  const newEmail = String(decoded?.newEmail || "")
    .trim()
    .toLowerCase();

  if (
    tokenPurpose !== EMAIL_CHANGE_TOKEN_PURPOSE ||
  ...
  try {
    ...
    const currentDbEmail = String(customer.Email || "")
      .trim()
      .toLowerCase();
    ...
    if (currentDbEmail !== currentEmail) {
      if (currentDbEmail === newEmail) {
        ...
        return res.json({
          success: true,
          message:
            "Email is already updated and verified. You can continue using your account.",
        });
      }
      ...
      return res.status(400).json({
        error:
          "This email change link is no longer valid. Request a new email change from profile settings.",
    ...
    return res.json({
      success: true,
      message:
        "Email address updated and verified successfully. You can continue using your account.",
    });
  } catch (error) {
    if (!transaction.finished) {
      await transaction.rollback();
...
const isPasswordResetTableMissingError = (error) => {
  ...
  return (
    (mysqlCode === "ER_NO_SUCH_TABLE" && message.includes("password_reset")) ||
...
exports.register = async (req, res) => {
  try {
    const { name, email, phone, password, profileImageUrl, ProfileImageURL } =
      req.body;
    ...
    if (existingStaff) {
      return res
        .status(409)
        .json({ error: "This email is already used by a staff account" });
    }
...
exports.resendEmailVerification = async (req, res) => {
  try {
    ...
    const issueResult = await issueVerificationEmail(customer, {
...
exports.requestPasswordReset = async (req, res) => {
  try {
    ...
    const genericMessage =
      "If the email exists, an OTP has been sent to your registered contact methods.";

    if (!email || !userType) {
      return res
        .status(400)
        .json({ error: "Email and user type are required" });
    }
    ...
    if (deliveryTasks.length > 0) {
      const deliveryResults = await Promise.allSettled(
        deliveryTasks.map((entry) => entry.task),
      );
      ...
      if (failedDeliveries.length > 0) {
        console.error(
          "Password reset OTP delivery had failures:",
          failedDeliveries,
        );
      }
      ...
      if (skippedChannels.length > 0) {
        console.warn(
          "Password reset OTP delivery skipped channels:",
          skippedChannels,
        );
      }
      ...
      if (successfulChannels.length === 0) {
        console.error(
          "Password reset OTP was generated but no delivery channel reported success for:",
          normalizedEmail,
        );
      }
    }
...
exports.verifyResetOTP = async (req, res) => {
  try {
    ...
    if (!email || !otp || !userType) {
      return res
        .status(400)
        .json({ error: "Email, OTP, and user type are required" });
    }
